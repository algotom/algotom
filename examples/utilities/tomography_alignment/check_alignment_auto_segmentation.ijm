...
The macro is for auto-segmentation of the sphere.

Author: Nghia Vo (NSLS-2, DLS) and Robert Atwood (DLS).
 */
close("*");
// Display dimensions and crop ratio
displayw=800;
displayw1=1000;
cropratio=0.25;
// Minimum and maximum number of files allowed
minnumfile=12;
maxnumfile=361;
...
if (answer==1){
	...
	setTool("rectangle");
  	...
  	if (answer1==1){
  		...
  		run("Make Binary", "method=Default background=Light calculate black");
  		id4=getImageID();
  		answer2=getBoolean("The sample is a ball?");
  		run("Set Measurements...", "  center bounding redirect=None stack decimal=3");
		run("Analyze Particles...", "size=400-Infinity circularity=0.0-1.00 show=Nothing display clear include stack");	  		
		noelement=nSlices;
		noparticle=nResults;
		if (noelement!=noparticle){
			exit(" There are more than 2 or no round-object in the images.\n Please use the macro with manually adjusting threhold!!!");
		}
