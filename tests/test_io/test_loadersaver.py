...
class LoaderSaverMethods(unittest.TestCase):
    ...
    def test_load_image(self):
        ...
        mat = losa.load_image(file_path)
        self.assertTrue(len(mat.shape) == 2)
        self.assertRaises(Exception, losa.load_image, "./tmp1/")
        self.assertRaises(ValueError, losa.load_image, "./tmp1\\")
    ...
    def test_load_hdf(self):
        ...
        self.assertRaises(Exception, losa.load_hdf, "./tmp/data2.hdf",
                          "entry/data")
        self.assertRaises(ValueError, losa.load_hdf, "./tmp/data.hdf",
                          "entry/data2")

    def test_make_folder_name(self):
        folder = "./tmp/new_folder/"
    ...
    def test_save_image(self):
        file_path = "./tmp/test_overwrite/img.tif"
        losa.save_image(file_path, np.float32(np.random.rand(64, 64)))
        ...
        losa.save_image(file_path, np.float32(np.random.rand(64, 64)),
                        overwrite=False)
        num_file = len(losa.find_file("./tmp/test_overwrite/*tif"))
    ...
    def test_get_hdf_tree(self):
        ...
        ifile.create_dataset("entry/energy", data=25.0)
        ifile.close()
        entries = losa.get_hdf_tree(file_path, output="./tmp/tree.txt",
                                    display=False)
        self.assertTrue(
            os.path.isfile("./tmp/tree.txt") and ("data" in entries[2]))

    def test_get_reference_sample_stacks_dls(self):
        num_stack = 3
        for i in range(num_stack):
            file_path = "./tmp/speck_tomo_" + str(i) + ".hdf"
            ifile = h5py.File(file_path, "w")
            ...
            ifile.close()
        list_path = losa.find_file("./tmp/speck_tomo*")
    ...
    def test_load_image_multiple(self):
        num_img = 4
        for j in range(num_img):
            file_path = "./tmp/multi/img_" + str(j) + ".tif"
            losa.save_image(file_path, np.ones((64, 64), dtype=np.float32))
        list_path = losa.find_file("./tmp/multi/*tif")
        img_stack = losa.load_image_multiple(list_path)
        self.assertTrue(num_img == img_stack.shape[0])
